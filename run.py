...
class posterior_probability_computation :
  ...
  def q1_beta(self,A,w,partition_2,a_1_transformed) :
      ...
      numerator = len(self.intersection(self.intersection(A,P_betaw),a_1_transformed))
  ...
  def find_sets1(self,A,w,a_1_transformed,partition_2,q1) :
      ...
      for k in range(len(partition_2)) :
          set_k = partition_2[k] 
          numerator = len(self.intersection(self.intersection(A,set_k),a_1_transformed))
  ...
  def qt_alpha(self,A,w,partition_1,b_prec_transformed) :
      ...
      """
      P_alphaw = self.P(w,partition_1)
      return len(self.intersection(self.intersection(A,P_alphaw),b_prec_transformed))/len(self.intersection(P_alphaw,b_prec_transformed)), self.intersection(P_alphaw,b_prec_transformed)
  ...
  def a_t(self,A,w,b_prec_transformed,partition_1,qt) :
      ...
      for k in range(len(partition_1)) :
          set_k = partition_1[k]
          numerator = len(self.intersection(self.intersection(A,set_k),b_prec_transformed))
  ...
  def qt_beta(self,A,w,partition_2,a_t_transformed) :
      ...
      numerator = len(self.intersection(self.intersection(A,P_betaw),a_t_transformed))
  ...
  def b_t(self,A,w,a_t_transformed,partition_2,qt_beta) :
      ...
      for k in range(len(partition_2)) :
          set_k = partition_2[k]
          numerator = len(self.intersection(self.intersection(A,set_k),a_t_transformed))
    ...
    for i in range(len(partition_1)) :
      for j in range(len(partition_2)) :
        if len(self.intersection(partition_1[i],partition_2[j])) > 0 :
            joint_partition_output.append(self.intersection(partition_1[i],partition_2[j]))
  ...
  def run(self):
    ...
    self.join_initial_partition = self.joint_partition(self.partition_1,self.partition_2)

    # Run

    q1_alpha_proba, partition_ini_alpha = self.q1_alpha(self.A,self.w,self.partition_1)
    list_qt_alpha_proba.append(q1_alpha_proba)

    ### Prior for beta computation
    elt_part_2 = self.P(self.w,self.partition_2)
    list_qt_beta_proba.append(len(self.intersection(self.A,elt_part_2))/len(elt_part_2))
    
    ### First step 

    a_1 = self.find_sets0(self.A,self.w,self.partition_1,q1_alpha_proba)
    a_1_transformed = self.transforms_sets_of_sets(a_1)
    q1_beta_proba, partition_ini_beta = self.q1_beta(self.A,self.w,self.partition_2,a_1_transformed)
    self.partition_1 = a_1
    b_1 = self.find_sets1(self.A,self.w,a_1_transformed,self.partition_2,q1_beta_proba) 
    b_1_transformed = self.transforms_sets_of_sets(b_1) 
    self.partition_2 = b_1
    ...
    qt_alpha_proba, partition_alpha = self.qt_alpha(self.A,self.w,self.partition_1,b_before)
    at = self.a_t(self.A,self.w,b_before,self.partition_1,qt_alpha_proba)
    a_t_transformed = self.transforms_sets_of_sets(at)
    self.partition_1 = at
    qt_beta_proba, partition_beta = self.qt_beta(self.A,self.w,self.partition_2,a_t_transformed)
    bt = self.b_t(self.A,self.w,a_t_transformed,self.partition_2,qt_beta_proba)
    b_before = self.transforms_sets_of_sets(bt)
    self.partition_2 = bt

    b_before_before = b_1_transformed
    a_t_transformed_before = a_1_transformed



    if b_before == b_before_before and a_t_transformed_before == a_t_transformed :
        return list_qt_alpha_proba, list_qt_beta_proba, len(self.joint_partition([partition_alpha],self.join_initial_partition))
    
    t = 2
    # All following steps 
    while b_before != b_before_before and a_t_transformed_before != a_t_transformed: 

        t += 1
        b_before_before = b_before
        a_t_transformed_before = a_t_transformed
    ...
    list_qt_alpha_proba.append(qt_alpha_proba)
    list_qt_beta_proba.append(qt_beta_proba)
    #print(list_qt_alpha_proba,list_qt_beta_proba)
    return list_qt_alpha_proba, list_qt_beta_proba, t
